import { ACTIVE_BIOME_PROFILES, BIOME_BLEND_TUNING, BIOME_SELECTION_TUNING, TERRAIN_OCTAVE_TUNING, } from "./Biomes.js";
export class Chunk {
    ...
    constructor(centerX, centerZ, size, deltaMap = new Map(), isNether = false) {
        this.x = centerX;
        this.z = centerZ;
        this.size = size;
    ...
    // 3D Perlin noise
    perlinNoise3D(worldX, worldY, worldZ, octave, frequency) {
    ...
    sampleBiomeProfileAt(worldX, worldZ) {
        const selector = this.clamp01(this.sampleValueNoise(worldX, worldZ, BIOME_SELECTION_TUNING.selectorOctave, BIOME_SELECTION_TUNING.selectorFrequency)) * 0.999999;
        ...
        if (frac < transitionWidth || frac > 1 - transitionWidth) {
            ...
            return this.blendBiomeProfiles(ACTIVE_BIOME_PROFILES[lowerIdx], ACTIVE_BIOME_PROFILES[upperIdx], blendFactor);
        }
    ...
    sampleHeightAtWorld(worldX, worldZ, gridSizes, multCoeffs) {
        const octaves = Math.min(gridSizes.length, multCoeffs.length);
        const sampleHeightAtPoint = (sampleX, sampleZ) => {
            ...
            for (let octave = 0; octave < octaves; octave++) {
                ...
                const noiseVal = this.sampleValueNoise(sampleX, sampleZ, octave, frequency);
                sum += noiseVal * coeff;
                maxPossibleHeight += coeff;
            }
            const normalized = sum / maxPossibleHeight;
            const shaped = this.smoothstep(BIOME_BLEND_TUNING.defaultShapeLow, BIOME_BLEND_TUNING.defaultShapeHigh, normalized);
    ...
    generateCubes() {
        ...
        if (this.isNether) {
            ...
            this.heightMapData = new Float32Array(this.size * this.size).fill(Chunk.NETHER_CEILING);
        }
        else {
            // Overworld: biome-driven multi-octave height map.
            const activeGridSizes = [...TERRAIN_OCTAVE_TUNING.gridSizes];
            const activeMultCoeffs = [...TERRAIN_OCTAVE_TUNING.multCoeffs];
            this.heightMapData = new Float32Array(this.size * this.size);
            for (let i = 0; i < this.size; i++) {
                for (let j = 0; j < this.size; j++) {
                    const worldX = topLeftX + j;
                    const worldZ = topLeftZ + i;
                    this.heightMapData[this.size * i + j] = this.sampleHeightAtWorld(worldX, worldZ, activeGridSizes, activeMultCoeffs);
                }
            }
            ...
            for (let i = 0; i < this.size; i++) {
                for (let j = 0; j < this.size; j++) {
                    ...
                    if (pondNoise < -0.15) {
                        ...
                        this.heightMapData[this.size * i + j] = Math.max(1, terrainH - carveDepth);
                    }
                }
            }
        }
        ...
        for (let k = 0; k < this.size * this.size; k++) {
            if (this.heightMapData[k] > maxH)
                maxH = this.heightMapData[k];
        }
        ...
        for (let i = 0; i < this.size; i++) {
            for (let j = 0; j < this.size; j++) {
                ...
                for (let y = 0; y < colMaxY; y++) {
                    if (this.getLocalCubeType(i, j, y) !== Chunk.blockTypeAir &&
        ...
        for (const [key, blockType] of this.deltaMap) {
            ...
            const colMaxY = this.getColMaxY(i, j);
            if (y >= colMaxY && this.isExposed(i, j, y))
                this.cubes++;
        }
    ...
    blockTypeAt(worldX, y, worldZ, columnHeight, biome) {
        // Height = 0: bedrock
        if (y == 0) {
        ...
        const noise = 0.7 * this.perlinNoise3D(worldX, y, worldZ, 100, 0.1) +
    ...
    netherBlockTypeAt(worldX, y, worldZ, columnHeight) {
        ...
        const heightFrac = this.clamp01((y - Chunk.NETHER_LAVA_LEVEL) /
    ...
    computeVertexAO(i, j, y, out, idx) {
        const opaqueAt = (ci, cj, cy) => this.isOpaqueAt(ci, cj, cy);
        ...
        // Face 0: Top (+Y) — normal dy=+1, t1=j(X), t2=i(Z)
        {
            ...
            for (let c = 0; c < 4; c++) {
                ...
                const s2 = c & 2 ? 1 : -1;
                fb |=
                    aoCorner(opaqueAt(i, j + s1, ny), opaqueAt(i + s2, j, ny), opaqueAt(i + s2, j + s1, ny)) <<
                        (c * 2);
            }
            float0 += fb;
        }
        // Face 1: Left (-X) — normal dj=-1, t1=i(Z), t2=y(Y)
        {
            ...
            for (let c = 0; c < 4; c++) {
                ...
                const s2 = c & 2 ? 1 : -1;
                fb |=
                    aoCorner(opaqueAt(i + s1, nj, y), opaqueAt(i, nj, y + s2), opaqueAt(i + s1, nj, y + s2)) <<
                        (c * 2);
            }
            float0 += fb * 256;
        }
        ...
        // Face 3: Front (+Z) — normal di=+1, t1=j(X), t2=y(Y)
        {
            ...
            for (let c = 0; c < 4; c++) {
                ...
                const s2 = c & 2 ? 1 : -1;
                fb |=
                    aoCorner(opaqueAt(ni, j + s1, y), opaqueAt(ni, j, y + s2), opaqueAt(ni, j + s1, y + s2)) <<
                        (c * 2);
            }
            float1 += fb;
        }
    ...
    isOpaqueAt(i, j, y) {
        if (i < 0 || i >= this.size || j < 0 || j >= this.size)
            return false;
        if (y < 0)
            return true; // below world is always opaque
        const t = this.getLocalCubeType(i, j, y);
        if (t === Chunk.blockTypeAir)
            return false;
        if (t >= Chunk.blockTypeWaterFlowLevel3 &&
    ...
    isExposed(i, j, y) {
        if (!this.isOpaqueAt(i, j, y + 1))
            return true; // above
        if (y === 0 || !this.isOpaqueAt(i, j, y - 1))
            return true; // below
        if (!this.isOpaqueAt(i - 1, j, y))
            return true;
        if (!this.isOpaqueAt(i + 1, j, y))
            return true;
        if (!this.isOpaqueAt(i, j - 1, y))
            return true;
        if (!this.isOpaqueAt(i, j + 1, y))
            return true;
        return false;
    }
    updateCubePositionsAndTypes() {
        const [topLeftX, topLeftZ] = this.origin();
     ...
     */
    topBlockAt(worldX, worldZ, yMaxInclusive = 100) {
        const [topLeftX, topLeftZ] = this.origin();
     ...
     */
    static circleOverlapsBlockColumn(px, pz, r, ix, iz) {
     ...
     */
    static verticalCapsuleIntersectsSolids(solidAt, px, headY, pz, radius, capsuleHeight) {
    ...
    static worldToChunkAxis(worldVal, chunkSize) {
        return Math.floor((worldVal + chunkSize / 2) / chunkSize) * chunkSize;
    }
    static isSolidBlockWorldWide(getChunk, wx, wy, wz) {
    ...
    static cylinderIntersectsSolidWorld(getChunk, px, headY, pz, radius, capsuleHeight) {
        return Chunk.verticalCapsuleIntersectsSolids((ix, iy, iz) => Chunk.isSolidBlockWorldWide(getChunk, ix, iy, iz), px, headY, pz, radius, capsuleHeight);
    }
    static supportedHeadYWorld(getChunk, px, pz, feetY, radius, capsuleHeight, footSlack) {
    ...
    static separateVerticalCapsuleFromSolids(getChunk, px, py, pz, radius, capsuleHeight, vy) {
        const solid = (hx, hy, hz) => Chunk.cylinderIntersectsSolidWorld(getChunk, hx, hy, hz, radius, capsuleHeight);
        if (!solid(px, py, pz))
            return py;
        const step = 0.025;
        const maxSteps = 400;
     ...
     * penetration).
     */
    static resolveUpwardPenetration(getChunk, px, py, pz, radius, capsuleHeight) {
        if (!Chunk.cylinderIntersectsSolidWorld(getChunk, px, py, pz, radius, capsuleHeight)) {
            return py;
        }
        const sepStep = 0.025;
        let y = py;
        let guard = 0;
        while (Chunk.cylinderIntersectsSolidWorld(getChunk, px, y, pz, radius, capsuleHeight) &&
            guard < 400) {
            y -= sepStep;
            guard++;
        }
    ...
    static tryHorizontalCylinderMove(getChunk, px, py, pz, dx, dz, radius, capsuleHeight) {
        const solid = (x, z) => Chunk.cylinderIntersectsSolidWorld(getChunk, x, py, z, radius, capsuleHeight);
        if (!solid(px + dx, pz + dz))
            return { ax: dx, az: dz };
        if (dx !== 0 && !solid(px + dx, pz))
            return { ax: dx, az: 0 };
        if (dz !== 0 && !solid(px, pz + dz))
            return { ax: 0, az: dz };
        return { ax: 0, az: 0 };
    }
    /**
     ...
     * intersects solid.
     */
    static verticalCapsuleHasHeadroomForJump(getChunk, px, headY, pz, radius, capsuleHeight) {
        for (let dh = 0; dh <= 1; dh += 0.35) {
            if (Chunk.cylinderIntersectsSolidWorld(getChunk, px, headY + dh, pz, radius, capsuleHeight)) {
                return false;
            }
        }
        return true;
    ...
    isSolidBlockAtWorld(wx, wy, wz) {
        ...
        if (this.isWater(wx, wz, wy))
            return false;
        if (this.isLava(wx, wz, wy))
            return false;
        return true;
    }
    /**
     ...
     */
    supportTopUnderFeet(columnWorldX, columnWorldZ, feetY, footSlack, yMaxInclusive = 100) {
        let colMax = -Infinity;
     ...
     * new delta map.
     */
    changeCubeType(worldX, worldZ, worldY, newType) {
        const [topLeftX, topLeftZ] = this.origin();
        const cubeChunkX = Math.round(worldX - topLeftX);
        const cubeChunkZ = Math.round(worldZ - topLeftZ);
        const cubeChunkY = Math.round(worldY);
        this.deltaMap.set(this.deltaKey(cubeChunkX, cubeChunkZ, cubeChunkY), newType);
        this.updateCubePositionsAndTypes();
        return this.deltaMap;
    }
    changeCubeTypeNoUpdate(worldX, worldZ, worldY, newType) {
        const [topLeftX, topLeftZ] = this.origin();
        const cubeChunkX = Math.round(worldX - topLeftX);
        const cubeChunkZ = Math.round(worldZ - topLeftZ);
        const cubeChunkY = Math.round(worldY);
    ...
    isWater(worldX, worldZ, worldY) {
        const t = this.cubeType(worldX, worldZ, worldY);
        return (t === Chunk.blockTypeWater ||
    ...
    isFlowWater(worldX, worldZ, worldY) {
        const t = this.cubeType(worldX, worldZ, worldY);
        return (t === Chunk.blockTypeWaterFalling ||
            (t !== undefined &&
                t >= Chunk.blockTypeWaterFlowLevel3 &&
                t <= Chunk.blockTypeWaterFlowLevel1));
    }
     ...
     */
    applyCubeTypeChanges(blocks, overwriteSolid = false) {
        if (blocks.length === 0) {
            return this.deltaMap;
        }
        const [topLeftX, topLeftZ] = this.origin();
        ...
        for (const block of blocks) {
            ...
            const currentType = this.getLocalCubeType(cubeChunkZ, cubeChunkX, cubeChunkY);
            ...
            this.deltaMap.set(this.deltaKey(cubeChunkX, cubeChunkZ, cubeChunkY), block.type);
            changed = true;
        }
        if (changed) {
            this.updateCubePositionsAndTypes();
...
Chunk.blockTypeAir = -1;
Chunk.blockTypeDirt = 0;
Chunk.blockTypeCobble = 1;
Chunk.blockTypeWater = 2;
Chunk.blockTypeWaterFalling = 99; // water rushing straight down
Chunk.blockTypeWaterFlowLevel3 = 100; // most water, 1 step from source/falling
Chunk.blockTypeWaterFlowLevel2 = 101; // 2 steps out
Chunk.blockTypeWaterFlowLevel1 = 102; // least water, max spread (does not spread further)
Chunk.blockTypeCoalOre = 3;
Chunk.blockTypeIronOre = 4;
Chunk.blockTypeGoldOre = 5;
Chunk.blockTypeDiamondOre = 6;
Chunk.blockTypeGrass = 7;
Chunk.blockTypeSand = 8;
Chunk.blockTypeSandstone = 9;
Chunk.blockTypeSnow = 10;
Chunk.blockTypeNetherite = 11;
Chunk.blockTypeBedrock = 12;
Chunk.blockTypePortal = 13;
Chunk.blockTypePortalFrame = 30;
Chunk.blockTypeLava = 14;
Chunk.blockTypeNetherRack = 15;
Chunk.SEA_LEVEL = 8;
Chunk.NETHER_LAVA_LEVEL = 4;
Chunk.NETHER_CEILING = 58;
Chunk.seedHash = 2166136261 >>> 0;
