...
(function (ItemAction) {
    ...
    ItemAction[ItemAction["Use"] = 1] = "Use";
    ItemAction[ItemAction["Place"] = 2] = "Place";
    ItemAction[ItemAction["Equip"] = 3] = "Equip";
})(ItemAction || (ItemAction = {}));
export class ItemType {
    constructor(id, name, image, maxStackSize) {
        this.id = id;
        this.name = name;
    ...
    getBlockType() {
        return this.actionType === ItemAction.Place ? this.action : -1;
    }
}
export var itemTypes = new Map();
export function registerItemTypes() {
    const registerItem = (id, name, maxStackSize = 16) => {
    ...
    registerItem("wood", "Wood").setAction(ItemAction.Place, DecorationGenerator.blockTypeWood);
    registerItem("bedrock", "Bedrock").setAction(ItemAction.Place, Chunk.blockTypeBedrock);
    ...
    registerItem("blaster", "Blaster", 1).setAction(ItemAction.Use, (app, pos) => {
        app.fireBlaster();
    });
    registerItem("nether_star", "Nether Star", 1).setAction(ItemAction.Use, (app, pos) => {
    ...
    registerItem("food", "Food").setAction(ItemAction.Use, (app, pos) => {
        app.player.eat(5);
        const count = app.inventory.getHeldItem().count;
        app.inventory.editSlotCount(app.inventory.selectedHotbarIdx, count - 1);
    });
...
export class Inventory {
    ...
    static slotIndex(x, y) {
        return y * Inventory.width + x;
    }
    static equipmentIndex(i) {
        return 1000 + i;
    }
    editSlotCount(index, count) {
        if (count <= 0) {
    ...
    hasEquipment(itemType) {
        return (this.equipmentSlots.filter((i) => i && i.itemType.id === itemType.id)
            .length > 0);
    }
    hasEquipmentById(itemId) {
        const itemType = itemTypes.get(itemId);
        if (!itemType) {
            throw Error("Item type not found: " + itemType);
        }
    ...
    canFitStack(itemStack) {
        ...
        for (const stack of this.items) {
            ...
            else if (stack.itemType.id === itemType.id &&
    ...
    gridOrigin(canvasWidth, canvasHeight) {
        ...
        const invX = layoutX + equipW + Inventory.INV_PADDING * 2 + Inventory.PANEL_GAP;
        const invY = (canvasHeight - totalHeight) / 2;
        return [invX, invY];
    }
    /** Draws a single inventory slot by index. */
    drawSlot(ctx, x, y, index, highlight) {
    ...
    drawHotbar(ctx, canvasWidth, canvasHeight) {
        ...
        const selectedItem = this.getItemStack(Inventory.slotIndex(this.selectedHotbarIdx, 0));
        ctx.font = "16px monospace";
        ...
        for (let i = 0; i < cols; i++) {
            this.drawSlot(ctx, i * (SLOT_SIZE + SLOT_GAP), 0, Inventory.slotIndex(i, 0), i === this.selectedHotbarIdx);
        }
        ctx.restore();
    }
    /** Draws a labeled panel background. */
    drawPanel(ctx, x, y, w, h, label, centerLabel = false) {
    ...
    drawInventoryScreen(ctx, canvasWidth, canvasHeight, mouseX, mouseY) {
        ...
        this.forEachInventorySlot((col, invRow, x, y) => {
            this.drawSlot(ctx, x, y, Inventory.slotIndex(col, invRow), invRow === 0 && col === this.selectedHotbarIdx);
        });
        ...
        for (let i = 0; i < Inventory.equipmentCount; i++) {
            this.drawSlot(ctx, equipX, equipY + i * (SLOT_SIZE + SLOT_GAP), Inventory.equipmentIndex(i), false);
        }
    ...
    craftingLayout(panelX, panelY, panelW, panelH) {
        const innerPad = 14;
    ...
    drawCraftingContents(ctx, panelX, panelY, panelW, panelH) {
        ...
        ctx.fillText(`Output: ${recipe.outputItemId} x${recipe.outputCount}`, layout.detailX + 12, layout.detailY + 82);
        // Craft button
        ...
        ctx.fillText(canCraft ? "Craft (Enter)" : "Need more materials", layout.craftButtonX + layout.craftButtonW / 2, layout.craftButtonY + layout.craftButtonH / 2);
        ctx.restore();
    }
    selectCraftingRecipe(delta) {
    ...
    craftSelectedRecipe() {
        ...
        const inserted = this.insertItemById(recipe.outputItemId, recipe.outputCount);
        if (inserted) {
            this.itemsCrafted++;
        }
        return inserted;
    ...
    //   }
    // }
    isInSlot(relX, relY, x, y) {
        const { SLOT_SIZE } = Inventory;
        return (relX >= x && relX < x + SLOT_SIZE && relY >= y && relY < y + SLOT_SIZE);
    }
    handleClick(mouseX, mouseY, canvasWidth, canvasHeight, button) {
        const { SLOT_SIZE, SLOT_GAP, INV_PADDING, PANEL_GAP, CRAFTING_PANEL_HEIGHT, CRAFTING_PANEL_GAP, } = Inventory;
        const [originX, originY] = this.gridOrigin(canvasWidth, canvasHeight);
    ...
    handleCraftingClick(relX, relY, panelX, panelY, panelW, panelH) {
        const layout = this.craftingLayout(panelX, panelY, panelW, panelH);
        // Recipe list selection
        if (relX >= layout.recipeListX &&
    ...
    clickSlot(index, button) {
        const slotItem = this.getItemStack(index);
        const canUseSlot = !this.mouseItem ||
            !(1000 <= index &&
                index < 2000 &&
...
Inventory.width = 9;
Inventory.height = 4;
Inventory.equipmentCount = 2;
Inventory.SLOT_SIZE = 60;
Inventory.SLOT_GAP = 10;
Inventory.INV_PADDING = 15;
Inventory.HOTBAR_GAP = 30;
Inventory.PANEL_GAP = 50;
Inventory.CRAFTING_PANEL_HEIGHT = 350;
Inventory.CRAFTING_PANEL_GAP = 75;
