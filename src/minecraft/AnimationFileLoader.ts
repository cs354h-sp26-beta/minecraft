import {
  ColladaLoader,
  Collada,
} from "../lib/threejs/examples/jsm/loaders/ColladaLoader.js";
import {
  Object3D,
  Scene,
  MeshLambertMaterial,
...
export class MeshGeometryLoader {
  ...
  constructor(geometry: BufferGeometry, wMat: Mat4) {
    ...
    this.position = new AttributeLoader(
      gPosition.array as Float32Array,
      gPosition.count,
      gPosition.itemSize,
    );
    let gNormal = geometry.attributes.normal;
    this.normal = new AttributeLoader(
      gNormal.array as Float32Array,
      gNormal.count,
      gNormal.itemSize,
    );
    if (geometry.attributes.uv) {
      let gUV = geometry.attributes.uv;
      this.uv = new AttributeLoader(
        gUV.array as Float32Array,
        gUV.count,
        gUV.itemSize,
      );
    } else {
    ...
    this.skinIndex = new AttributeLoader(
      gSkinIndex.array as Float32Array,
      gSkinIndex.count,
      gSkinIndex.itemSize,
    );
    let gSkinWeight = geometry.attributes.skinWeight;
    this.skinWeight = new AttributeLoader(
      gSkinWeight.array as Float32Array,
      gSkinWeight.count,
      gSkinWeight.itemSize,
    );

    this.v0 = new AttributeLoader(
      new Float32Array(gPosition.array.length),
      gPosition.count,
      4,
    );
    this.v1 = new AttributeLoader(
      new Float32Array(gPosition.array.length),
      gPosition.count,
      4,
    );
    this.v2 = new AttributeLoader(
      new Float32Array(gPosition.array.length),
      gPosition.count,
      4,
    );
    this.v3 = new AttributeLoader(
      new Float32Array(gPosition.array.length),
      gPosition.count,
      4,
    );
...
export class MeshLoader {
  ...
  constructor(skinnedMesh: SkinnedMesh) {
    ...
    this.geometry = new MeshGeometryLoader(
      skinnedMesh.geometry as BufferGeometry,
      this.worldMatrix,
    );
    this.bones = [];
    this.boneIndices = [];
