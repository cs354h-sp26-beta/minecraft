...
export class Bone {
    ...
    public boneToWorldTransform(): Mat4 {
        return new Mat4().setIdentity().translate(this.position).multiply(this.rotation.toMat4());
    }

    public worldToBoneTransform(): Mat4 {
        return this.rotation.toMat4().transpose().translate(this.position.copy().scale(-1));
    }
    ...
    public updateRecursively(): void {
        ...
        this.endpoint = this.rotation.multiplyVec3(this.endpointLocal).add(this.position);
...
export class Mesh {
    ...
    constructor(mesh: MeshLoader | Mesh) {
        this.geometry = (mesh instanceof Mesh) ? mesh.geometry : new MeshGeometry(mesh.geometry);
        ...
        if (this.bones.length > 64) {
            console.log(`Warning: More than 64 bones in rig (${this.bones.length}), shader may not function.`);
        }
    ...
    public scale(scaling: number) {
        for (let arr of [this.geometry.position.values, this.geometry.v0.values, this.geometry.v1.values, this.geometry.v2.values, this.geometry.v3.values]) {
    ...
    public setPose(pose: Quat[], blendAmount: number = 0) {
        for (let i = 0; i < this.bones.length; i++) {
            Quat.slerpShort(pose[i], this.bones[i].relativeRot, blendAmount, this.bones[i].relativeRot)
        }
