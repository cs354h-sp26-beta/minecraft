...
export function registerItemTypes() {
  ...
  registerItem("grass", "Grass").setAction(ItemAction.Place, Chunk.blockTypeGrass);
  ...
  registerItem("coal", "Coal", 64);
  registerItem("iron", "Iron", 64);
  registerItem("gold", "Gold", 64);
  registerItem("diamond", "Diamond", 64);
  registerItem("stick", "Stick", 64);
  registerItem("ammo", "Ammo", 64);
  registerItem("boots", "Jump Boots", 1);
  registerItem("jetpack", "Jetpack", 1);
  registerItem("blaster", "Blaster", 1);
}
  ...
  static dropsFrom(blockType: number): ItemStack | null {
    switch (blockType) {
      ...
      case Chunk.blockTypeGrass:
        return new ItemStack(itemTypes.get("grass")!, 1);
...
export class Inventory {
  ...
  public removeItemById(itemId: string, count: number): boolean {
    if (this.countItem(itemId) < count) {
      return false;
    }
    ...
    for (let i = 0; i < this.items.length && count > 0; i++) {
      ...
      const used = Math.min(stack.count, count);
      stack.count -= used;
      count -= used;
      if (stack.count <= 0) {
        this.items[i] = null;
  ...
  public addItemById(itemId: string, count: number = 1): boolean {
  ...
  public canFitItemById(itemId: string, count: number): boolean {
    ...
    const probe = new ItemStack(itemType, count);
    return this.canFitStack(probe);
  }
  ...
  public canFitStack(itemStack: ItemStack | null): boolean {
    ...
    for (const stack of this.items) {
      if (!stack) {
        count -= Math.min(itemType.maxStackSize, count);
        if (count <= 0) {
          return true;
        }
      }
